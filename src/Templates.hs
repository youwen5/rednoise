    ...
    do
      ...
      return $ docTypeHtml ! lang "en" $ do
        H.head $ do
          ...
          -- "$if(pagetitle)$ $pagetitle$ $else$ $if(slug)$ Youwen Wu >> $slug$ $else$\n    Youwen Wu >> $title$ $endif$ $endif$"
        ...
        body
          ...
          $ do
            aside ! class_ "hidden md:block w-64 flex-none" $ do
              a
                ! href "/"
            ...
            H.div ! class_ "flex-1 md:mt-2" $ do
              header ! class_ "md:hidden border-b border-dashed border-muted mb-8 pb-8 w-full" $ do
              ...
              main ! class_ "main-content lg:max-w-[40rem]" $ do
                ...
                H.div ! class_ "space-y-1 text-center mt-4" $ do
                  case date of
                    Just date' -> p ! class_ "text-subtle text-md md:text-lg" $ toHtml date'
                    Nothing -> return ()

                -- "$if(date)$"
                -- p ! class_ "text-subtle text-md md:text-lg" $ "$date$"
                -- "$endif$\n          $if(author)$"
                -- p ! class_ "text-lg md:text-xl mt-5" $ do
                --   em "by"
                --   "$author$"
                -- "$endif$"
                case author of
                  Just author' -> p ! class_ "text-lg md:text-xl mt-5" $ em "by " >> toHtml author'
                  Nothing -> return ()
                ...
                H.div
                  ...
                  $ preEscapedToHtml
                  $ itemBody item
                footer ! class_ "border-t mt-8 border-solid border-muted mb-4 text-sm text-muted py-1" $ do
                  p ! class_ "all-smallcaps leading-[1.3]" $ do
                    ...
                    a
                      ...
                      $ "e5814475"
                    "using rustc 1.93.0-nightly at 2025-12-04 17:03:40 utc-07. Most content\n    CC-BY-SA-4.0. This page uses"
